...
class Resumen
{
    ...
    public void CrearReporte(string FechaInicio, string FechaFin, string encabezado1, string encabezado2)
    {
        try
        {
            ...
            string cadenapdf2 = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Resumen");
            if (!Directory.Exists(cadenapdf2))
            {
                Directory.CreateDirectory(cadenapdf2);
            }

            string nombreArchivo = "Resumen_" + Datetime + ".pdf";
            ...
            CelTabTitulos = new PdfPCell(new Phrase(encabezado2, _FuenteTitulos10NB));
            CelTabTitulos.Rowspan = 2;
            CelTabTitulos.VerticalAlignment = Element.ALIGN_CENTER;
            CelTabTitulos.HorizontalAlignment = 1; //0=Left, 1=Centre, 2=Right       
            CelTabTitulos.Border = 0;
            ...
            PdfPCell CelConceptos = new PdfPCell(new Phrase("Fecha", _FuenteConceptos7W));
            ...
            CelConceptos = new PdfPCell(new Phrase("", _FuenteConceptos7W));
            CelConceptos.Rowspan = 2;
            CelConceptos.VerticalAlignment = Element.ALIGN_LEFT;
            CelConceptos.HorizontalAlignment = 0; //0=Left, 1=Centre, 2=Right
            CelConceptos.BackgroundColor = new ClsColoresReporte(Predial10.Properties.Settings.Default.colorfactura).color;
            ...
            try
            {
                ...
                SQL1 = " select fecha, sum(importe) as total from recibomaestro inner join reciboesclavo on recibomaestro.serie = reciboesclavo.serie and recibomaestro.folio = reciboesclavo.recibo where recibomaestro.Cancelado = 'A'  and fecha between '" + FechaInicio + "' and '" + FechaFin + "' group by fecha";
                ...
                {
                    clave = DateTime.Parse( datosconcepto.Rows[j]["fecha"].ToString()).ToString("dd/MM/yyyy");
                    ...
                    CelDato = new PdfPCell(new Phrase("", _FuenteNormal7B));
                    CelDato.Rowspan = 2;
                    CelDato.VerticalAlignment = Element.ALIGN_LEFT;
                    CelDato.HorizontalAlignment = 0; //0=Left, 1=Centre, 2=Right       
                    CelDato.Border = 2;
