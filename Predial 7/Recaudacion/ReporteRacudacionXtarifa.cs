...
class reportexTarifaITSharp
{
    ...
    public void CrearReporte( string FechaInicio, string FechaFin, string encabezado1, string encabezado2)
    {
        try
        {
            ...
            string cadenapdf2 = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "ReportexTarifa");
            if (!Directory.Exists(cadenapdf2))
            {
                Directory.CreateDirectory(cadenapdf2);
            }

            string nombreArchivo = "ReportexTarifa_"+ Datetime + ".pdf";
          ...
          //  byte[] logoempresa = Conexion_a_BD.obtenerimagen("select logo from empresa");
            ...
            CelTabTitulos = new PdfPCell(new Phrase("Reporte por Tarifa", _FuenteTitulos10));
            CelTabTitulos.Rowspan = 2;
            CelTabTitulos.VerticalAlignment = Element.ALIGN_CENTER;
            CelTabTitulos.HorizontalAlignment = 1; //0=Left, 1=Centre, 2=Right       
            CelTabTitulos.Border = 0;
            ...
            float[] widthsConceptos = new float[] { 300f, 100f, 200f };
            ...
            PdfPCell CelConceptos = new PdfPCell(new Phrase("Tarifa", _FuenteConceptos7W));
            ...
            CelConceptos = new PdfPCell(new Phrase("No de Recibo", _FuenteConceptos7W));
            CelConceptos.Rowspan = 2;
            CelConceptos.VerticalAlignment = Element.ALIGN_LEFT;
            CelConceptos.HorizontalAlignment = 0; //0=Left, 1=Centre, 2=Right
            CelConceptos.BackgroundColor = new ClsColoresReporte(Predial10.Properties.Settings.Default.colorfactura).color;;
            ...
            try
            {
                ...
                SQL1 = "select Tarifa, count(idrecibomaestro) as cuantos, sum(total) as Total from recibomaestro where fecha between '" + FechaInicio + "' and '" + FechaFin + "' and cancelado = 'A' group by tarifa";
                ...
                {
                    Tarifa = datosconcepto.Rows[j]["Tarifa"].ToString();
                    cuantos = Int64.Parse(datosconcepto.Rows[j]["cuantos"].ToString());
                    ...
                    float[] widthsesp = new float[] { 300f, 100f, 200f };
                    TabDatos.DefaultCell.Border = iTextSharp.text.Rectangle.NO_BORDER;
                    TabDatos.SetTotalWidth(widthsesp);

                    PdfPCell CelDato = new PdfPCell(new Phrase(Tarifa, _FuenteNormal7B));
                    CelDato.Rowspan = 2;
                    ...
                    CelDato = new PdfPCell(new Phrase(cuantos.ToString(), _FuenteNormal7B));
                    CelDato.Rowspan = 2;
